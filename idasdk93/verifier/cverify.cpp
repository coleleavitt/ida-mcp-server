...
struct cinsn_verifier_t : public ctree_parentee_t
{
  ...
  int idaapi visit_expr(cexpr_t *e) override { func.verify_expr(parent_item(), e); return 0; }
};
      ...
      if ( maturity < CMAT_TRANS1 || maturity >= CMAT_CASTED )
      {
        ...
        if ( jea != BADADDR && i->ea != jea )
          CFAIL_QASSERT(50683, i); // ctree: mismatch in if-statement and its expression addresses
                                   // call recalc_item_addresses() to fix it
      }
      // no break
    case cit_for:
    case cit_while:
...
void cfunc_t::verify(allow_unused_labels_t _aul, bool always) const
{
  ...
  {
    flags64_t F32 = get_flags32(mba->entry_ea);
    if ( !is_func(F32) )
...
//-------------------------------------------------------------------------
#ifdef TESTABLE_BUILD
void *citem_t::operator new(size_t /*size*/, void *ptr) //lint !e3436 does not throw std::bad_alloc
{
