...
//--------------------------------------------------------------------------
static AS_PRINTF(1, 2) void dmsg(const char *format, ...)
{
  va_list va;
  va_start(va, format);
#if 1
...
//--------------------------------------------------------------------------
static bool create_ptr(ea_t ea)
{
...
//--------------------------------------------------------------------------
static ea_t get_pointer_value(ea_t ea)
{
...
static bool find_impdir(uunp_ctx_t &ctx, range_t *impdir)
{
  ...
  for ( ea_t pos = ctx.curmod.start_ea;
        ...
        pos += sizeof(DWORD) )
  {
    pos = bin_search(pos, ctx.curmod.end_ea, (uchar *)&ea32, nullptr, 4,
    ...
    for ( int k=0; k < 2; k++ )
    {
      ...
      while ( true )
      {
        ...
        ea_t func = ctx.is_9x ? ctx.win9x_find_thunk(ea) : get_pointer_value(ea);
        if ( func == 0 )
          continue;
...
static bool create_impdir(uunp_ctx_t &ctx, const range_t &impdir)
{
  ...
  {
    dmsg("UUNP: import entry %a\n", ea);
    create_ptr(ea);
    ea_t func = ctx.is_9x ? ctx.win9x_find_thunk(ea) : get_pointer_value(ea);
    ssize_t nlen = get_name(&buf, func);
    dmsg("UUNP: import name %a len=%" FMT_ZS " %s\n", func, nlen, buf.c_str());
    if ( nlen <= 0 )
      continue;
    ...
    if ( !force_name(ea, name, SN_IDBENC) )
      dmsg("%a: cannot rename to imported name '%s'\n", ea, name);
  }
...
static void create_impdir(uunp_ctx_t &ctx)
{
  ...
  dmsg("Uunp: Import directory bounds %a..%a\n", impdir.start_ea, impdir.end_ea);
  ...
  switch ( code )
  {
    ...
    case dbg_library_load:
      if ( stage == 0 )
      {
        ...
        dmsg("UUNP: loaded %s, waiting for %s\n", modname, myname);
    ...
    case dbg_library_unload:
      if ( stage != 0 && is_dll )
      {
        ...
        if ( curmod.start_ea == pev->modinfo().base
          || curmod.start_ea == pev->modinfo().rebase_to )
        {
          dmsg("UUNP: unload unpacked module\n");
          if ( stage > 2 )
...
FORCE_STOP:
      ...
      dmsg("UUNP: set a breakpoint at %a (stage %d)\n", bp_gpa, stage);
      continue_process();
      break;
    ...
    case dbg_bpt:      // A user defined breakpoint was reached.
                       ...
                       //               1 - to always display a breakpoint warning dialog.
      {
        ...
        dmsg("UUNP: hit breakpoint at %a (stage %d)\n", ea, stage);
        if ( stage == 2 )
        {
          if ( ea == bp_gpa )
          {
            ...
            if ( get_sp_val(&esp) )
            {
              ...
              ea_t gpa_caller = get_pointer_value(esp);
              if ( !is_library_entry(*this, gpa_caller) )
              {
                ...
                dmsg("UUNP: nameaddr=%a name=%s\n", nameaddr, name.c_str());
                if ( !ignore_win32_api(name.c_str()) )
                {
                  dmsg("%a: found a call to GetProcAddress(%s)\n", gpa_caller, name.c_str());
          ...
          else if ( ea == bpt_ea )
          {
            ...
            {
              dmsg("UUNP: reached unpacker code at %a, switching to trace mode\n", ea);
...
bool idaapi uunp_ctx_t::run(size_t arg)
{
  if ( arg == 2 )
  {
    ...
    {
      dmsg("Invalid import address table boundaries\n");
      return true;
    }
