...
// Compare two hex_place_t objects
// Return -1, 0, 1
bool ida_export hex_place_t__equals(const hp_t *ths, const place_t *t2, void *)
{
  return hex_place_t__compare(ths, t2) == 0;
}
...
bool ida_export hex_place_t__deserialize(hex_place_t *_this, const uchar **pptr, const uchar *end)
{
  if ( !place_t__deserialize(_this, pptr, end) || *pptr >= end )
    return false;
  memory_deserializer_t mmdsr(*pptr, end);
  _this->n = mmdsr_unpack_ea(mmdsr);
  *pptr = mmdsr.ptr;
  return true;
}
