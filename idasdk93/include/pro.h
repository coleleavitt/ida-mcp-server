...
/// Get string representation of a time_t (local time)
...
#ifndef SWIG
  /// Construct and append a new element to the end the qvector with a
...
/// \return code
/// \retval 0 if two strings are identical
/// \retval > 0 if 's1' is larger than 's2'
/// \retval < 0 if 's2' is larger than 's1'
/// \retval otherwise return first nonzero comparison between chars in 's1' and 's2'
///@{
...
//---------------------------------------------------------------------------
#define streq(s1, s2)          (::qstrcmp((s1), (s2))  == 0)           ///< convenient check for string equality
#define strneq(s1, s2, count)  (::qstrncmp((s1), (s2), (count))  == 0) ///< see #streq
...
/// \return true-replaced some tabs
...
///
/// Note that this does not tell us anything regarding the number of bytes
/// that compose the byte sequence; 1-byte sequences (i.e., ASCII) are still
/// UTF-8 byte sequences.
///
/// \param in the byte to check
/// \return success
...
/// \return success (i.e., all bytes converted)
...
/// \return the number of codepoints that we failed to decode, thus:
...
/// \return how many bytes were put into the output buffer
...
/// If `optname` is provided, the `-O<optname>:` token will be prepended
/// \param[out] out  the output buffer
/// \param opts      the plugin options to serialize
/// \param optname   the optional option name to prepend to the plugin options string
...
{
  size_t cb = sizeof(*this);     ///< size of this structure
  int flags = 0;                 ///< \ref LP_
...
#define LP_NO_ASLR        0x0040 ///< disable ASLR (mac and linux)
...
#define LP_REPLACE_ENV    0x0800 ///< completely replace env for process
///@}
  const char *path = nullptr;    ///< file to run
  const char *args = nullptr;    ///< command line arguments
  ssize_t in_handle = -1;        ///< handle for stdin or -1
  ssize_t out_handle = -1;       ///< handle for stdout or -1
  ssize_t err_handle = -1;       ///< handle for stderr or -1
  char *env = nullptr;           ///< zero separated environment variables that will be appended
  ...
  const char *startdir = nullptr;///< current directory for the new process
  void *info = nullptr;          ///< os specific info (on windows it points to PROCESS_INFORMATION)
                                 ///< on unix, not used
};
...
/// \return 0-ok, otherwise an error code that can be passed to winerr()
...
/// \param[out] read_handle handle from which the caller will read. can be nullptr
/// \param[out] write_handle handle to which the caller will write. can be nullptr
/// \param      lpp         process to run
/// \param[out] errbuf      buffer for the error message, if any
/// \return nullptr on error, otherwise the process handle
...
/// \return the physical core count, or -1 on error

idaman THREAD_SAFE int ida_export get_physical_core_count();

/// Get the total CPU logical core count
/// \return the logical core count, or -1 on error

idaman THREAD_SAFE int ida_export get_logical_core_count();

/// Get the number of logical CPU cores available to the current process
/// if supported by the OS.
/// \return the logical core count available for the process, or -1 on error
